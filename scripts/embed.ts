...
(async function () {
  ...
  try {
    ...
    // 🎯 FIXED: Cast navigator safely to LegacyNavigator to support older properties without 'any'
    const legacyNavigator = navigator as LegacyNavigator;

    const metadataPayload = {
      userAgent: navigator.userAgent,
      language: navigator.language || legacyNavigator.userLanguage || 'en',
    ...
    const initData = await initResponse.json() as { 
      status: string; 
      data?: { visitor?: { trackingId?: string } } 
    }
    ...
    const handleMessage = (event: MessageEvent) => {
      ...
      const data = event.data as { type?: string; count?: number; width?: string; height?: string } | null
      if (!data || typeof data !== 'object') return

      switch (data.type) {
        case 'UNREAD_UPDATE': {
          const currentState = iframe.getAttribute('data-state')
          if (currentState === 'minimized' && data.count !== undefined) {
