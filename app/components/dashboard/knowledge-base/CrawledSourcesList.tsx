...
export default function CrawledSourcesList({
  ...
  return (
    <>
      {/* ✅ The wrapper Card with id always remains in the DOM so scrolling hooks never break */}
      <Card
      ...
      >
        <div className="p-4 border-b border-border bg-sidebar/5">
          <Text fw={600} size="sm" className="text-foreground">
            Crawled Website Index Records ({sources ? sources.length : 0})
          </Text>
        </div>

        {!sources || sources.length === 0 ? (
          /* ✅ Clean inline empty placeholder state */
          <Group justify="center" gap="xs" p="xl" className="text-dimmed py-12">
            <FiLink size={18} />
            <Text size="sm">No custom website paths indexed yet.</Text>
          </Group>
        ) : (
