...
import { sendOperatorMessage, sendTypingStatus } from '@/app/lib/api/chat.api'
import { useAuthStore } from '@/app/store/useAuthStore'
import { useOperatorChatStore } from '@/app/store/useOperatorChatStore' // 👈 Import E2EE state engine
import { getChatSocket } from '@/app/hooks/useChatSocket'
...
export default function OperatorInput({ sessionId }: OperatorInputProps) {
  ...
  const operator = useAuthStore((state) => state.operator)
  const encryptOutboundText = useOperatorChatStore(
    (state) => state.encryptOutboundText,
  ) // 👈 Pull encryption router
  const socket = getChatSocket()
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      // 🔒 1. PROCESS CLIENT-SIDE END-TO-END CRYPTO WRAPPING
      let finalMessageText = trimmed
      let isEncrypted = false
      let localIv: string | undefined = undefined

      if (trimmed) {
        const encryptedRes = await encryptOutboundText(sessionId, trimmed)
        if (encryptedRes) {
          finalMessageText = encryptedRes.ciphertext
          localIv = encryptedRes.iv
          isEncrypted = true
        }
      }

      // 🔒 2. DISPATCH SCRAMBLED INTERCEPTED STRUCTS DOWN PIPELINE
      await sendOperatorMessage({
        sessionId,
        senderType: 'operator',
        senderId: operatorId,
    ...
    <div className="border-t border-border bg-card/80 p-3 md:p-4 backdrop-blur-md relative">
      ...
      <div className="flex items-end gap-2 md:gap-3 max-w-7xl mx-auto relative">
        ...
        <button
          type="button"
          onClick={isRecording ? stopRecording : startRecording}
          className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl transition cursor-pointer ${isRecording ? 'bg-red-600 text-white animate-pulse' : 'text-muted-foreground hover:bg-muted hover:text-foreground'}`}
        >
          {isRecording ? <FiSquare size={14} /> : <FiMic size={16} />}
        </button>
        <button
