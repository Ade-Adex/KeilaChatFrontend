...
//     // 🎯 Catch when any user ends the chat session to reset open pane view states
...
//     socket.on('session_status_changed', handleStatusChangedEvent) // 🎯 Linked custom payload drop engine directly
...
import { getChatSocket } from '@/app/hooks/useChatSocket'
import { getMyProperties, sendOperatorHeartbeat } from '@/app/lib/api/chat.api'
import type { OperatorConversation, ChatMessage } from '@/app/types/dashboard'
...
function HeartbeatManager() {
  useEffect(() => {
    ...
    const sendHeartbeatSignal = async () => {
      try {
        // 🎯 FIXED: Replaced raw fetch with your structured API client wrapper
        await sendOperatorHeartbeat()
      } catch (err) {
        console.error('[Heartbeat] Failed pinging background presence agent:', err)
      }
    }

    // Fire immediately upon mounting workspace layouts
...
export default function OperatorDashboard() {
  ...
  return (
    <div className="flex h-full w-full bg-background overflow-hidden relative">
      {/* 🎯 Background state manager injected cleanly here */}
      <HeartbeatManager />
