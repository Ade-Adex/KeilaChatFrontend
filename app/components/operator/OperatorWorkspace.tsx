...
import { usePropertySettings } from '@/app/hooks/usePropertySettings' // 🎯 ADDED: Use the new custom hook
import {
  closeSession,
  getActiveOperators,
  getSessionMessages,
...
export default function OperatorWorkspace({ session }: OperatorWorkspaceProps) {
  ...
  // 🎯 FIXED: One-liner abstracting all side-effects and data loading 
  const { features } = usePropertySettings(extractedPropertyId)
  ...
  }, [session, currentSessionId, socket, user?._id, extractedPropertyId])
  ...
  return (
    <div className="flex h-full flex-col bg-background/40 relative">
      ...
      <div className="relative z-10 bg-linear-to-t from-background via-background/90 to-transparent pt-4">
        ...
        {/* 🎯 UPDATED: Passing clean config settings managed dynamically by the hook */}
        <OperatorInput
          sessionId={currentSessionId}
          allowFileUpload={features.allowFileUpload}
          allowVoiceRecordings={features.allowVoiceRecordings}
        />
      </div>
