...
import { getChatSocket } from '@/app/hooks/useChatSocket'
import { useAuthStore } from '@/app/store/useAuthStore'
import { useOperatorChatStore } from '@/app/store/useOperatorChatStore' // 👈 E2EE Zustand Store Hook
import type { OperatorConversation, ChatMessage } from '@/app/types/dashboard'
import TypingIndicator from '@/app/components/TypingIndicator'
import {
  FiShuffle,
...
export default function OperatorWorkspace({ session }: OperatorWorkspaceProps) {
  ...
  // 🔒 Pull down multi-tenant E2EE hooks from our state layer
  const {
    initiateOperatorHandshake,
    handleVisitorPublicKey,
    decryptInboundMessage,
    exchangedSessions,
  } = useOperatorChatStore()
  const socket = getChatSocket()
  const currentSessionId = session._id
  const isKeyExchanged = exchangedSessions[currentSessionId] ?? false

  useEffect(() => {
    if (!socket.connected) socket.connect()
  }, [socket])

  // 🔒 1. TRIGGER E2EE HANDSHAKE EXCHANGE ROUTINE
  useEffect(() => {
    if (currentSessionId) {
      void initiateOperatorHandshake(currentSessionId)
    }
  }, [currentSessionId, initiateOperatorHandshake])

  // 🔒 2. INTERCEPT INBOUND HANDSHAKES & FORCE RETROACTIVE DECRYPTION RETRIES
  useEffect(() => {
    if (!socket.connected) return

    const handleKeyExchangeEvent = async (payload: {
      sessionId: string
      ...
      if (
        ...
        payload.clientType === 'visitor'
      ) {
        await handleVisitorPublicKey(currentSessionId, payload.publicKey)

        // Key established! Re-decrypt current memory array lines immediately
        setMessages((prev) =>
          prev.map((msg) => {
            if (msg.isEncrypted && msg.iv && !msg.isDecrypted) {
              void decryptInboundMessage(currentSessionId, msg).then(
                (clearMsg) => {
                  setMessages((curr) =>
                    curr.map((m) =>
                      m._id === msg._id
                        ? { ...clearMsg, isDecrypted: true }
                        : m,
                    ),
                  )
                },
    ...
    socket.on('public_key_received', handleKeyExchangeEvent)
    return () => {
      socket.off('public_key_received', handleKeyExchangeEvent)
    }
  }, [socket, currentSessionId, handleVisitorPublicKey, decryptInboundMessage])

  // 🔒 3. ASYNC DECRYPTION DEPLOYED ON HISTORICAL MOUNT RE-LOADS
  useEffect(() => {
    let mounted = true
    const fetchMessages = async () => {
      try {
        ...
        // Decrypt history lines using the workspace key states mapped over arrays
        const decryptedList = await Promise.all(
          fetchedMsgs.map(async (msg) => {
            if (msg.isEncrypted && msg.iv) {
              const parsed = await decryptInboundMessage(currentSessionId, msg)
              return { ...parsed, isDecrypted: true }
            }
            return msg
          }),
        )
        ...
        const possessesUnread = decryptedList.some(
          (m) => m.senderType === 'visitor' && m.status !== 'seen',
        )
        if (possessesUnread && socket.connected) {
          socket.emit('mark_session_seen', {
  ...
  }, [currentSessionId, socket, isKeyExchanged, decryptInboundMessage])
  ...
  // 🔒 4. DECRYPT REAL-TIME MESSAGES IN-FLIGHT
  useEffect(() => {
    const handleMessage = async (message: ChatMessage) => {
      ...
      if (message.isEncrypted && message.iv) {
        const parsed = await decryptInboundMessage(currentSessionId, message)
        processedMessage = { ...parsed, isDecrypted: true }
      }
  ...
  }, [currentSessionId, socket, session, decryptInboundMessage])
  ...
  const getVisitorDisplayName = () => {
    ...
    if (typeof session.visitorId === 'object') {
      if ('name' in session.visitorId && session.visitorId.name)
        return session.visitorId.name
      const structuredData = session.visitorId as StructuralVisitorWrapper
      if (structuredData.visitorId?.name) return structuredData.visitorId.name
    }
    return fallback
  }
