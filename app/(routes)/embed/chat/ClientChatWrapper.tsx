...
// import { useEmbedResize } from '@/app/hooks/useEmbedResize'
...
import { getChatSocket } from '@/app/hooks/useChatSocket'
import { useEmbedResize } from '@/app/hooks/useEmbedResize'
import { initiateSession, getSessionMessages } from '@/app/lib/api/chat.api'
...
export default function ClientChatWrapper({
  ...
  // 3. Fetch database message logs as soon as session context maps out
  useEffect(() => {
    // 1. Guard check handles potential null values
    if (!session?.sessionId) return

    // 2. Extract the id into a local constant so TypeScript knows it cannot become null
