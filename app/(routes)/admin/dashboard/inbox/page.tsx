...
export default function AdminInboxPage() {
  ...
  // Track continuous socket instances safely
  ...
  const selectedThread = activeThreads.find((t) => t.sessionId === selectedSessionId)
  const isSessionClosed = selectedThread?.status === 'closed'

  // Pipeline 1: Global Connection Initialization & Core Dashboard Listeners
  useEffect(() => {
    if (!user || !user.property?.id) return
    ...
    socketInstance.on('incoming_visitor_alert', (payload: IncomingAlertPayload) => {
      setActiveThreads((prev) => {
        const filtered = prev.filter((t) => t.sessionId !== payload.sessionId)
        const updated: ThreadSummary = {
          sessionId: payload.sessionId,
    ...
    // Fetch primary active workspace elements
    fetch(`${BACKEND_URL}/api/v1/sessions/property/${propertyId}`, {
      headers: { Authorization: `Bearer ${user.accessToken}` },
    })
      .then((res) => res.json())
      .then((resData: ApiResponse) => {
        if (resData.status === 'success' && resData.data.sessions) {
          const formatted: ThreadSummary[] = resData.data.sessions.map((s: BackendSession) => ({
            sessionId: s._id,
            propertyId: s.propertyId,
            lastMessageText: s.lastMessageText || 'No messages generated yet',
            updatedAt: s.updatedAt,
  ...
  // Pipeline 2: Contextual Multi-tenant Session Workspace Transition Pipeline
  useEffect(() => {
    if (!selectedSessionId || !user || !socket) return

    // Inform backend that the operator is stepping into this dynamic conversation thread
    ...
    // Reset localized view states
    setIsVisitorTyping(false)

    // Load conversation history archives
    fetch(`${BACKEND_URL}/api/v1/sessions/${selectedSessionId}/messages?propertyId=${user.property?.id}`, {
      headers: { Authorization: `Bearer ${user.accessToken}` },
    })
      .then((res) => res.json())
      .then((resData: HistoryResponse) => {
    ...
    // Set active stream data matrix listeners
    socket.on('presence_notification', (payload: MessagePayload) => {
      if (payload.sessionId === selectedSessionId) {
        setMessages((prev) => [...prev, payload])
      }
    })

    socket.on('user_typing', (payload: { senderName: string; isTyping: boolean }) => {
      if (payload.senderName !== user.name) {
        setIsVisitorTyping(payload.isTyping)
      }
    })

    socket.on('new_message', (payload: MessagePayload) => {
      if (payload.sessionId === selectedSessionId) {
        setIsVisitorTyping(false)
        setMessages((prev) => {
      ...
      // Live update dashboard panel state tracking matrices
      setActiveThreads((prev) =>
        prev.map((t) =>
          t.sessionId === payload.sessionId
            ? {
    ...
    socket.on('session_closed', (payload: { sessionId: string }) => {
      ...
      setActiveThreads((prev) =>
        prev.map((t) => (t.sessionId === payload.sessionId ? { ...t, status: 'closed' } : t))
      )
    })
  ...
  const sendResponse = () => {
    if (!socket || !operatorInput.trim() || !selectedSessionId || !user) return
  ...
  const handleEndSession = async () => {
    ...
    try {
      const response = await fetch(`${BACKEND_URL}/api/v1/sessions/${selectedSessionId}/end`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${user.accessToken}`,
      ...
      if (response.ok) {
        setSelectedSessionId(null)
        setActiveThreads((prev) => prev.filter((t) => t.sessionId !== selectedSessionId))
      }
    } catch (err) {
      console.error('Failed to end session:', err)
    }
  ...
  return (
    <div className="flex h-[calc(100vh-60px-var(--mantine-spacing-md)*2)] w-full bg-background text-foreground font-sans select-none overflow-hidden rounded-xl border border-border shadow-sm">
      {/* SIDEBAR ACTIVE QUEUE COLUMN */}
      <div className={`w-full md:w-85 border-r border-border flex flex-col justify-between ${selectedSessionId ? 'hidden md:flex' : 'flex'}`}>
        <div className="flex flex-col flex-1 overflow-hidden">
          <div className="p-4 border-b border-border flex items-center justify-between bg-background">
            <h3 className="text-sm font-semibold tracking-tight">Live Conversations</h3>
            <span className="text-[11px] bg-blue-500/10 text-primary font-medium px-2 py-0.5 rounded-full">
              {activeThreads.filter((t) => t.status !== 'closed').length} active
            </span>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-3 space-y-1.5 custom-scrollbar">
            {isLoadingThreads ? (
              <div className="flex flex-col gap-2.5">
                {[1, 2, 3, 4].map((i) => (
                  <div key={i} className="w-full p-4 bg-neutral-100 dark:bg-neutral-800/50 rounded-xl space-y-2 animate-pulse">
                    <div className="flex justify-between items-center">
                      <div className="h-4 bg-neutral-200 dark:bg-neutral-700 rounded w-1/2" />
                      <div className="h-3 bg-neutral-200 dark:bg-neutral-700 rounded w-1/4" />
                    </div>
              ...
              activeThreads.map((thread) => {
                ...
                return (
                  ...
                  >
                    <div className={`h-9 w-9 rounded-lg flex items-center justify-center text-xs font-semibold shrink-0 ${isSelected ? 'bg-neutral-800 text-neutral-100 dark:bg-neutral-200 dark:text-neutral-800' : 'bg-button-hover group-hover:bg-background'}`}>
                      {shortId.slice(0, 2)}
                    </div>

                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-baseline mb-0.5">
                        <span className="text-sm font-semibold truncate">Visitor #{shortId}</span>
                        <span className={`text-[10px] ${isSelected ? 'opacity-80' : 'text-foreground'}`}>
                          {new Date(thread.updatedAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                        </span>
                      </div>
                      <p className={`text-xs truncate ${isSelected ? 'opacity-90' : 'text-foreground'}`}>
                        {thread.lastMessageText}
                      </p>
                      {thread.status === 'closed' && (
                        <span className="inline-block mt-1.5 text-[9px] uppercase tracking-wider font-bold bg-button-hover px-1.5 py-0.5 rounded">
              ...
              <div className="flex flex-col items-center justify-center h-64 text-center p-6">
                ...
                <p className="text-sm font-medium text-neutral-900 dark:text-neutral-100">Inbox is pristine</p>
                <p className="text-xs text-neutral-400 mt-1">
                  No incoming support sessions matching your property queue routing context.
                </p>
              </div>
            )}
          </div>
      ...
      {/* CORE ACTIVE CONVERSATION WORKSPACE */}
      <div className={`flex-1 flex flex-col bg-background ${selectedSessionId ? 'flex' : 'hidden md:flex'}`}>
        {selectedSessionId ? (
          <>
            <div className="p-4 border-b border-border flex items-center justify-between gap-3 shadow-[0_1px_2px_rgba(0,0,0,0.02)]">
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-0.5">
                    <span className="text-[11px] text-neutral-400 font-mono truncate">ID: {selectedSessionId}</span>
                    {isVisitorTyping && (
                      <span className="text-[10px] text-emerald-500 font-medium italic flex items-center gap-1">
                        <span className="animate-bounce">·</span>
                ...
                return Object.entries(groups).map(([dateLabel, groupMessages]) => (
                  <div key={dateLabel} className="flex flex-col gap-4">
                    ...
                    {groupMessages.map((msg, idx) => {
                      const isOp = msg.senderType === 'operator'
                      const isSys = msg.senderType === 'system'
                      const time = new Date(msg.createdAt || new Date()).toLocaleTimeString([], {
                        hour: '2-digit',
                        minute: '2-digit',
                      })
                      ...
                      if (isSys) {
                        return (
                          <div key={msg._id || idx} className="flex items-center justify-center w-full my-1">
                            <span className="text-[11px] bg-neutral-100 dark:bg-neutral-800 border border-border text-zinc-400 font-medium px-3 py-1 rounded-full text-center max-w-[85%]">
                              {msg.messageText}
                            </span>
                          </div>
                      ...
                      return (
                        <div key={msg._id || idx} className="flex flex-col group ${isOp ? 'items-end' : 'items-start'}">
                          <div className={`flex items-baseline gap-2 mb-1 text-[11px] text-foreground ${isOp ? 'flex-row-reverse' : 'flex-row'}`}>
                            <span className="font-medium text-foreground">{isOp ? 'You' : msg.senderName || 'Visitor'}</span>
                            <span className="opacity-70 text-[9px]">{time}</span>
                          </div>
            ...
            <div className="py-2 px-4 border-t border-border bg-card">
              <div className="flex items-center gap-2 rounded-xl transition-colors">
                <input
                  disabled={isSessionClosed}
                  value={operatorInput}
                  placeholder={isSessionClosed ? 'This conversation has been closed and archived...' : 'Type your secure reply here...'}
                  onChange={(e) => {
                    setOperatorInput(e.target.value)
                    sendTypingStatus(true)
                    if (typingTimerRef.current) clearTimeout(typingTimerRef.current)
          ...
          <div className="flex-1 flex flex-col items-center justify-center bg-neutral-50/20 dark:bg-neutral-950/5 p-8 text-center">
            ...
            <h3 className="text-sm font-semibold text-neutral-900 dark:text-neutral-100">No Thread Selected</h3>
            <p className="text-xs text-neutral-400 max-w-xs mt-1.5 leading-relaxed">
              Select a running operator pipeline room from the left dashboard routing grid columns matrix to interact.
            </p>
          </div>
          )}
      </div>
